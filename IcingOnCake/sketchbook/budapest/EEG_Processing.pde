...
class EEG_Processing_User {
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    // BETA - 13Hz to 30 or 12Hz to 39Hz
    // Wide awake – sensory motor awareness. Focused and alert, analytical thinking, learning new information quickly, complex mental processing.
    // Usually associated with peak mental and physical performance. Also associated with stress and anxiety. Can be used to enhance the absorption
    // and processing of new information.
    // THETA - 4Hz to 7.5Hz
    // Dream state, creative inspirations, hypnogogic state, enhanced visualization, deep meditation, sleep spindles, deep relaxation, inner peace
    // and sense of well being, long term memory. Believed to increase inner wisdom, enhanced intuition, reduce stress and transformational help for
    // limiting beliefs. May negatively affect attention disorders such ADD and ADHD
    // EMG - 7Hz to 20Hz?
    byte maxFreq = 7, minFreq = 4;
    
    float perFreqVal[][] = new float[maxFreq - minFreq][nchan];
    ...
    for (int Ichan=0;Ichan < nchan; Ichan++) {
      ...
      //println ("Ichan = " + Ichan + "; specSize = " + fftBuff[Ichan].specSize());
      ...
      for (int Ibin=0; Ibin < fftBuff[Ichan].specSize(); Ibin++){
        ...
        if (FFT_freq_Hz < minFreq || FFT_freq_Hz >= maxFreq) continue;
        
        //println("EEG_Processing_User: Ichan = " + Ichan + ", Freq = " + FFT_freq_Hz + "Hz, FFT Value = " + FFT_value_uV + "uV/bin");
        perFreqVal[(int) FFT_freq_Hz - minFreq][Ichan] = FFT_value_uV;
      }
    }
    
    SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss");
    for (int i = 0; i < maxFreq - minFreq; i++) {
      int valIdx = findMax(perFreqVal[i]);
      
      //println(sdf.format(new Date()) + "; freq = " + (i + minFreq) + "; val = " + perFreqVal[i][valIdx]);
    }
    /**/
  }
}
...
class EEG_Processing {
  ...
  private int currentNotch_ind = 1;  // set to 0 to default to 60Hz, set to 1 to default to 50Hz
  float data_std_uV[];
  float polarity[];
