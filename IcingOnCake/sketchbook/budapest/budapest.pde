...
String openBCI_portName = "/dev/ttyUSB1";  //starts as N/A but is selected from control panel to match your OpenBCI USB Dongle's serial/COM
int openBCI_baud = 115200; //baud rate from the Arduino


//here are variables that are used if loading input data from a CSV text file...double slash ("\\") is necessary to make a single slash
String playbackData_fname = "/tmp/openbci.log";
...
public int eegDataSource = 0; //default to none of the options
...
String output_fname;
String fileName = "/tmp/openbci.log";
...
void setup() {
  ...
  /*
  try {
    println("OpenBCI_GUI.pde:  attempting to open serial port for data output = " + serial_output_portName);
    ...
    println("OpenBCI_GUI.pde: *** ERROR ***: Could not open " + serial_output_portName);
  }
  /**/
...
void initSystem() {
  ...
  for (int i=0; i<nDataBackBuff; i++) dataPacketBuff[i] = new DataPacket_ADS1299(nchan, n_aux_ifEnabled);
  ...
  println("OpenBCI_GUI: initSystem: -- Init 1 --");
  ...
  println("OpenBCI_GUI: initSystem: -- Init 2 --");
  ...
  println("OpenBCI_GUI: initSystem: -- Init 3 --");
  ...
  println("OpenBCI_GUI: initSystem: -- Init 4 --");

  nextPlayback_millis = millis(); //used for synthesizeData and readFromFile.  This restarts the clock that keeps the playback at the right pace.

  startRunning();
  //sync GUI default settings with OpenBCI's deflt settings...
  //openBCI.syncWithHardware(sdSetting); //this starts the sequence off ... read in OpenBCI_ADS1299 iterates through the rest based on the ASCII trigger "$$$"
  println("OpenBCI_GUI: initSystem: -- Init 5 [COMPLETE] --");
}
  ...
  openBCI.updateSyncState(sdSetting);
    ...
    if (isRunning) {
      ...
      if (pointCounter >= nPointsPerUpdate) {
        ...
        OscMessage myMessage = new OscMessage("/bci");
        myMessage.add(System.currentTimeMillis());
        for (int Ival = 0; Ival < dataPacketBuff[curDataPacketInd].values.length; Ival++) {
          myMessage.add(openBCI.get_scale_fac_uVolts_per_count() * float(dataPacketBuff[curDataPacketInd].values[Ival]));
        }
        for (int Ival = 0; Ival < dataPacketBuff[curDataPacketInd].auxValues.length; Ival++) {
          myMessage.add(openBCI.get_scale_fac_uVolts_per_count() * float(dataPacketBuff[curDataPacketInd].auxValues[Ival]));
        }
    ...
    if (current_millis >= nextPlayback_millis) {
      ...
      for (int i = 0; i < nPointsPerUpdate; i++) {
        ...
        case DATASOURCE_SYNTHETIC: //use synthetic data (for GUI debugging)   
          ...
          break;
        case DATASOURCE_PLAYBACKFILE: 
          lastTimestamp = getPlaybackDataFromTable(playbackData_table, currentTableRowIndex, openBCI.get_scale_fac_uVolts_per_count(), dataPacketBuff[lastReadDataPacketInd]);
          if (lastTimestamp == 0) currentTableRowIndex = 0; 
          else currentTableRowIndex++;  
          break;
        default:
          //no action
        }
      ...
      if (eegDataSource == DATASOURCE_PLAYBACKFILE) println("OpenBCI_GUI: getDataIfAvailable: lastTimestamp = " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.S").format(new Date(lastTimestamp)));
...
void stopRunning() {
  ...
  println("OpenBCI_GUI: stopRunning: stop running...");
...
long getPlaybackDataFromTable(Table datatable, int currentTableRowIndex, float scale_fac_uVolts_per_count, DataPacket_ADS1299 curDataPacket) {
  float val_uV = 0.0f;
  ...
  //get each value
  for (int Ichan=0; Ichan < nchan; Ichan++) {
    if (Ichan + 1 < datatable.getColumnCount()) {
      val_uV = row.getFloat(Ichan + 1);
    } else {
  ...
  return new java.lang.Long(row.getString(0));
}
...
void oscEvent(OscMessage theOscMessage) {
  ...
  fileoutput.writeRawData_OSC(theOscMessage);
  
  OscMessage myMessage = new OscMessage("/bci");
  myMessage.add(new Long(System.currentTimeMillis()));
  // data.typetag() outputs a list of args as a string "sf" means one string + one float
  if (theOscMessage.addrPattern().startsWith("/videostart/")) {
    if (!theOscMessage.typetag().isEmpty()) {
      println("OutputFile_rawtxt: writeRawData_OSC received videostart argument with strange args, ignoring ...");
      return;
    }
    String[] addrPatternArr = theOscMessage.addrPattern().split("/", 3);
    
    myMessage.add(addrPatternArr[1]);
    myMessage.add(addrPatternArr[2]);
  }
  oscP5.send(myMessage, myNetAddressList);
}